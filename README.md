...
live). The catalog is **seventeen pools**: films, classic TV, anime, cartoons, old time radio,
...
| `GET /sitemap.xml` | Sitemap index: 25 static pages + every catalog item across all seventeen pools, split into one sub-sitemap per pool (~73k URLs, built from the local catalog indexes) |
...
URLs, picks one at random, and 302-redirects — uniform over every item in all seventeen pools
...
`changelog.md`): real searches return real legal films (catalog expanded to seventeen
...
the sitemap covers 73,125 URLs (all seventeen pools, split into per-pool sub-sitemaps, with `<lastmod>`),
