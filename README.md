...
live). The catalog is **fourteen pools**: films, classic TV, anime, cartoons, old time radio,
music & concerts, documentaries, sports, shorts, silent films, public broadcasting, science &
medicine, government films (FedFlix), and audiobooks (LibriVox) — every pool under the same
license gate (`lib/archive.ts`). The detail path re-verifies the license from full metadata (with a search-index
...
| `GET /sitemap.xml` | Sitemap: 22 static pages + every catalog item across all fourteen pools, deduped (~64k URLs, built from the local catalog indexes) |
...
URLs, picks one at random, and 302-redirects — uniform over every item in all fourteen pools
...
`changelog.md`): real searches return real legal films (catalog expanded to fourteen
...
the sitemap covers 64,088 URLs (all fourteen pools, deduped, with `<lastmod>`),
