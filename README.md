...
(`feature_films`, `prelinger`, `moviesandfilms` — union ≈ 18,489 legal-marked films, measured
live). The detail path re-verifies the license from full metadata (with a search-index
...
| `GET /sitemap.xml` | Sitemap: 6 static pages + the full ~18,489-film legal catalog (built from the local catalog index) |
...
URLs, picks one at random, and 302-redirects — uniform over the full legal catalog
(~18,489 films; the sitemap lists every catalog film). One random per request; rate-limited
...
`changelog.md`): real searches return real legal films (catalog expanded to ~18,489
legal-marked films across `feature_films`/`prelinger`/`moviesandfilms`), edge cache hits cut
...
the sitemap covers 18,495 URLs (the full legal catalog, with `<lastmod>`),
